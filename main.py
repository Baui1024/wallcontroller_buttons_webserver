...
def get_ip_config():
    if request.method == "POST":
        ...
        if new_config.get("gateway", "") != "" and not ip_tools.is_valid_ip(
            new_config.get("gateway", "")
        ):
            response.status = 400
        ...
        return {
            "status": "success",
            "config": new_config,
            "message": "Success! Restarting network in 1.5 seconds.",
        }
    else:
...
def security_config():
    if request.method == "POST":
        ...
        if new_config.get("accessControl") and not new_config.get("username"):
            ...
            return {
                "status": "error",
                "message": "Username required when access control is enabled",
            }
...
def login():
    ...
    # Verify credentials
    if username == security_config.get("username") and verify_password(
        password, security_config.get("passwordHash", "")
    ):
        # Create session
